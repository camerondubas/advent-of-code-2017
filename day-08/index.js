...
let output =  Math.max(...Object.values(input.trim().split('\n')
  .map(item =>
    item.split('if').map(
    ...
    prev.concat({
      target: current[0][0],
      increment: current[0][1] === 'inc',
      amount : Number(current[0][2]),
      condition: {
        target: current[1][0],
        comparator: current[1][1],
        value: current[1][2]
      }
    }), []
  ).reduce((results, step, index, self) => {
    if (!results[step.condition.target]) {
      results[step.condition.target] = 0;
    }

    if (!results[step.target]) {
      results[step.target] = 0;
    }

    let str = results[step.condition.target] + step.condition.comparator + step.condition.value;

    if (eval(str)) {
      results[step.target] += step.increment ? step.amount : -step.amount;
    }
...
let output2 = input.trim().split('\n')
  .map(item =>
    ...
    const str = results[step.condition.target] + step.condition.comparator + step.condition.value;
    if (eval(str)) {
      results[step.target] += step.increment ? step.amount : -step.amount;
    }

    const currentHighest = Math.max(...Object.values(results));
    if (currentHighest > results.highest) {
      results.highest = currentHighest;
    }
