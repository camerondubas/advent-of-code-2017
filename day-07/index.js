...
let a = input.trim().split('\n').map(item => item.split('->').map(item => item.trim().replace(/,/g, '').split(' ')))
  .reduce((prev, current) => {
    let obj = {
      name: current[0][0],
      weight: current[0][1],
  ...
  let b = a.reduce((prev,current) => {
    if (current.children) {
      let pairs = current.children.map(child => {
        let obj = {};
        obj[child] = current.name;
        return obj
      });
      return Object.assign({}, prev, ...pairs)
    }
    return prev
  }, {})

  let start = Object.keys(b)[0];
